...
class User:
    ...
    def validate_registration(form_data):
        ...
        if len(form_data['first_name']) < 2:
            ...
            flash("Need at least 2 characters in your first name, User.", "register")
            is_valid = False
        if len(form_data['last_name']) < 2:
            flash("Need at least 2 characters in your last name, User.", "register")
            is_valid = False
        # Compare input to regex
        if not User.EMAIL_REGEX.match(form_data['email']):
            flash("Invalid email address. Give it another go!", "register")
            is_valid = False
        # Check for repeat emails
        if User.has_repeats(form_data):
            flash("Email already exists. Give 'er another try!", "register")
            is_valid = False
        if not User.PASSWORD_REGEX.match(form_data['password']):
            flash("Users require the utmost security. Please use a password with 8-32 characters and at least 1 uppercase letter and 1 number.", "register")
            is_valid = False
        #Confirm if reentered password matches
        if form_data['confirm_password'] != form_data['password']:
            flash("Sorry User, passwords must match.", "register")
            is_valid = False
        return is_valid
